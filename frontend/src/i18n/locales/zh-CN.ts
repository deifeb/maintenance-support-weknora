...
export default {
  ...
  maintenance: {
    ...
    review: {
      ...
      list: {
        title: "需求清单审查",
        description: "查看正式审查状态、阻塞项和待处理项，并从当前已发布需求清单启动新的确定性审查。",
        register: "审查登记簿",
        total: "共 {count} 条审查",
        loading: "正在加载正式审查…",
        empty: "暂无正式需求审查。",
      ...
      source: {
        title: "启动正式审查",
        description: "来源仅来自服务端返回的当前 PUBLISHED 需求清单；浏览器只提交来源 ID 和预期版本。",
        currentPublished: "当前已发布需求清单",
        select: "请选择当前已发布清单",
        status: "状态",
        version: "乐观锁版本",
        current: "当前版本",
      ...
      summary: {
        ...
        blocking: "阻塞发现",
        pending: "待处理发现",
        pendingBlocking: "待处理阻塞发现",
      },
      finding: {
        title: "审查发现",
        description: "筛选、选择并处理正式 finding；批量决定由服务端原子校验。",
        all: "全部",
        key: "Finding Key",
        rule: "规则",
        severity: "严重度",
        blocking: "阻塞",
        blockingOnly: "仅阻塞",
        nonBlockingOnly: "仅非阻塞",
        admin: "高风险接受需管理员",
        decision: "决定状态",
        actions: "操作",
        empty: "当前筛选条件下没有 finding。",
      },
      decision: {
        title: "记录审查决定",
        action: "决定动作",
        finalQuantity: "最终数量",
        reason: "理由",
        highRisk: "该 finding 的接受或编辑接受属于高风险动作，需要管理员能力。",
      },
      validation: {
        adminRequired: "当前身份没有高风险接受权限。",
        reasonRequired: "EDIT_ACCEPTED 必须填写非空理由。",
        positiveQuantity: "EDIT_ACCEPTED 的最终数量必须为大于 0 的数字。",
      },
      detail: {
        title: "审查 #{id}",
        loadingTitle: "正在加载审查",
        loading: "正在加载正式审查详情…",
        invalidRoute: "无效的审查链接。",
        source: "来源需求清单",
        ruleSet: "规则集版本",
        version: "审查版本",
      },
      conflict: {
        ...
        actual: "实际版本",
        suggested: "建议动作",
        reloadHint: "重新加载服务端状态并重新确认决定。",
        reload: "重新加载服务端状态",
      },
      derive: {
        title: "派生新的需求清单 DRAFT",
        description: "仅在所有 blocking finding 已完成最终决定且具备 finalizeReview 权限时可执行。",
        confirmation: "确认根据已接受的正式审查决定派生新的需求清单 DRAFT？",
      },
      actions: {
        refresh: "刷新",
        run: "运行正式审查",
    ...
    scenario: {
      ...
      title: "情景方案向导",
      newTitle: "未命名任务情景",
      description: "按步骤确认任务边界、构型、任务阶段与计算策略；每个字段均保留来源、置信度和风险记录。",
      stepsLabel: "情景方案步骤",
      complete: "已完成",
      incomplete: "待完善",
      confirmed: "已确认",
      ...
      unsavedLeave: "当前仍有未保存的本地修改，确定离开吗？",
      steps: {
        basics: "任务基础",
        configuration: "装备与构型",
        mission: "任务剖面",
        reliabilityRepair: "可靠性与修理",
        calculation: "计算策略",
        confirmation: "确认与落库",
      },
      stepHints: {
        basics: "定义任务标识、时间窗口和优先级，形成所有后续计算的边界。",
        configuration: "选择已发布构型并定义参任务装备群，避免将未生效主数据带入计算。",
        mission: "把任务拆成可审计阶段，并为每个阶段分配在用数量和环境强度。",
        reliabilityRepair: "确认可靠性证据与修理策略；AI 推断值必须由人员确认。",
        calculation: "设定服务水平、执行模式和缺参处理策略。",
        confirmation: "复核阻塞项与字段来源，保存一致后才可生成 DRAFT 版本。",
      },
      fields: {
        scenarioName: "情景名称",
        missionCode: "任务编码",
        ...
        stageCode: "阶段编码",
        stageName: "阶段名称",
        durationHours: "持续时长（小时）",
        activeQuantity: "在用数量",
        maintenanceLevel: "维修级别",
        reliabilityProfiles: "可靠性剖面",
        repairPolicy: "修理策略",
        shockPolicy: "共因冲击策略",
        serviceLevel: "服务水平",
        executionMode: "执行模式",
      ...
      actions: {
        ...
        evidence: "项证据",
        applyFleet: "应用装备群",
        reloadServer: "重新加载服务器版本",
        discardLocal: "放弃本地修改",
        materialize: "生成 DRAFT 版本",
        materializing: "正在生成…",
      },
      conflict: {
        title: "服务器版本已更新",
        description: "本地草稿不会被覆盖。比较两侧版本后，选择重新加载或明确放弃本地修改。",
        local: "本地修改",
        server: "服务器版本",
      },
      confirmation: {
        ...
        blocked: "尚不能生成版本",
        ready: "所有发布前条件已满足",
        readyHint: "下一步只会生成可编辑的 DRAFT；正式发布仍需管理员二次确认。",
      },
      materialization: {
        draftCreated: "已生成 DRAFT 版本",
        failed: "生成版本失败",
      },
      list: {
        description: "浏览情景档案、版本状态与最后更新时间，并进入版本明细。",
        refresh: "刷新",
        create: "新建情景",
        search: "搜索情景",
        searchPlaceholder: "输入编码或名称",
        apply: "查询",
        includeInactive: "包含停用情景",
        records: "条记录",
        emptyTitle: "暂无情景方案",
        emptyDescription: "创建第一个任务情景，或调整当前筛选条件。",
        code: "情景编码",
        name: "情景名称",
        category: "分类",
        currentVersion: "当前版本",
      ...
      detail: {
        ...
        selectVersionHint: "从左侧版本登记册进入完整快照；已发布版本保持只读。",
        publishConfirmTitle: "确认发布版本",
        publishConfirmBody: "发布后该版本将成为正式计算输入。请确认已完成复核。",
        publishSuccess: "版本已发布",
        retireConfirmTitle: "确认退役版本",
        retireConfirmBody: "退役后该版本不再用于新任务计算，历史记录仍会保留。",
        retireSuccess: "版本已退役",
      },
      adminPublication: {
        title: "管理员发布",
        description: "只有管理员可将 DRAFT 发布为 PUBLISHED。",
      },
      empty: {
        fleetFirst: "请先选择构型并建立装备群，再分配阶段在用数量。",
        reliability: "暂无可用可靠性剖面，请先维护主数据。",
      },
      errors: {
        masterData: "主数据加载失败，请稍后重试。",
        draftLoad: "情景草稿加载失败。",
        materialize: "生成 DRAFT 版本失败。",
      },
    },
  },
  chatHeader: {
