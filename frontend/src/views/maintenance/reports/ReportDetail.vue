...
async function load(): Promise<void> { const current = ++request; const reportId = routeId.value; if (reportId === null) { loading.value = false; detail.value = null; versions.value = []; error.value = ''; notFound.value = false; return }; loading.value = true; error.value = ''; notFound.value = false; detail.value = null; versions.value = []; try { const [report, timeline] = await Promise.all([reportApi.getReport(reportId), reportApi.listReportVersions(reportId)]); if (current !== request) return; detail.value = report.data; versions.value = timeline.data } catch (reason) { if (current !== request) return; const normalized = normalizeMaintenanceError(reason); notFound.value = normalized.status === 404; error.value = notFound.value ? '' : normalized.message } finally { if (current === request) loading.value = false } }
