...
async function confirm(): Promise<void> { if (!props.allowed || submitting.value || props.busy || pendingReportMutations.has(props.reportId)) return; submitting.value = true; errorKey.value = ''; requestId.value = ''; const reportId = props.reportId; try { await runReportMutation(reportId, async () => { await reportApi.regenerateReport(reportId); await refreshReportReaders(reportId, props.refresh) }); emit('close') } catch (reason) { const error = normalizeMaintenanceError(reason); errorKey.value = reportErrorMessageKey(error.code); requestId.value = error.request_id ?? '' } finally { submitting.value = false } }
</script>
