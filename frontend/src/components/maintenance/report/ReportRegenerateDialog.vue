...
const props = defineProps<{ open: boolean; allowed: boolean; reportId: number; refresh: () => Promise<void> }>(); const emit = defineEmits<{ (event: 'close'): void }>(); const { t } = useI18n(); const submitting = ref(false); const errorKey = ref(''); const requestId = ref('')
async function confirm(): Promise<void> { if (!props.allowed || submitting.value) return; submitting.value = true; errorKey.value = ''; requestId.value = ''; try { await reportApi.regenerateReport(props.reportId); await props.refresh(); emit('close') } catch (reason) { const error = normalizeMaintenanceError(reason); errorKey.value = reportErrorMessageKey(error.code); requestId.value = error.request_id ?? '' } finally { submitting.value = false } }
</script>
