...
def test_report_service_builds_fixed_skeleton_and_finalizes_valid_report(
    ...
    assert len(sections) == len(get_template(AIReportType.MANAGEMENT_DECISION).sections) == 17
    assert {row.section_code for row in sections} == {
        section.code for section in get_template(AIReportType.MANAGEMENT_DECISION).sections
    }
    ...
    assert persisted.finalized_by == actor_admin.user_id


def test_report_service_builds_stocktake_template_skeleton(
    session,
    actor_admin,
) -> None:
    job = ai_report_service.create(
    ...
    sections = (
        ...
        .filter_by(report_version_id=version.id)
...
def test_report_service_rejects_foreign_linked_session_before_job_write(
    ...
    session.rollback()
    after = session.scalar(select(func.count(AIReportJob.id)))
    assert after == before
...
def test_report_export_authorizes_before_settings_access(
    ...
    def fail_settings_access():
        ...
        raise AssertionError("settings accessed before report ownership")
    ...
    assert session.scalar(select(func.count(AIReportExport.id))) == 0
...
def test_report_service_maps_create_job_ownership_miss_to_not_found(
    ...
    def raise_ownership_miss(*args, **kwargs):
        del args, kwargs
        raise LookupError("AISession 999 was not found")
