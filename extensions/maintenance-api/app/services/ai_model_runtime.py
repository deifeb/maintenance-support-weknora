...
class AIModelRuntime:
    ...
    def _candidate_decisions(
        ...
        route = self.router.registry.routes.get(
            request.function_name
        )
        if route is None:
            return []
        override = request.metadata.get(
            "model_override"
        )
        names: list[str] = []
        if override:
    ...
    def _start_call(
        ...
        *,
        ...
        definition = (
            self.router.registry.models.get(
                decision.selected
            )
        )
        provider_name = (
        ...
        try:
            row = self.repository.create_model_call(
                session,
                actor.tenant_id,
    ...
    def _finish_success(
        ...
        row.structured_validation_attempts = (
            metadata
            .structured_validation_attempts
        )
        row.fallback_used = (
    ...
    async def health(
        ...
        if self.router.rule_fallback is not None:
            health = (
                await self.router.rule_fallback
                .health_check()
            )
            results["RULE_FALLBACK"] = (
