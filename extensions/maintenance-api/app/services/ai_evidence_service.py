...
class WeknoraEvidenceRetriever:
    ...
    def _request_payload(
        ...
        return {
            "query": query.question,
            "equipment_model_id": (
                query.equipment_model_id
            ),
            "configuration_version_id": (
                query.configuration_version_id
            ),
            "spare_part_ids": query.spare_part_ids,
            "purpose": query.purpose,
            "valid_at": (
                query.valid_at.isoformat()
                if query.valid_at
                else None
            ),
        ...
        for raw in payload.get("items", []):
            try:
                ...
                entry = EvidenceEntry(
                    ...
                    parameter_name=raw.get(
                        "parameter_name"
                    ),
                    structured_value=raw.get(
                        "value",
                        raw.get(
                            "structured_value"
                        ),
                    ...
                    unit=raw.get("unit"),
                    applicable_equipment=raw.get(
                        "applicable_equipment"
                    ),
                    applicable_configuration=raw.get(
                        "applicable_configuration"
                    ),
                    effective_from=raw.get(
                        "effective_from"
                    ),
                    effective_to=raw.get(
                        "effective_to"
                    ),
                    source_name=str(
                        raw.get("source_name")
                    ...
                    source_document=raw.get(
                        "document"
                    ),
                    source_page=raw.get("page"),
                    knowledge_node=raw.get(
                        "knowledge_node"
                    ),
                    chunk_reference=raw.get(
                        "chunk_reference"
                    ),
                    retrieval_score=raw.get(
                        "score"
                    ),
                    rerank_score=raw.get(
                        "rerank_score"
                    ),
...
class AIEvidenceService:
    def __init__(
        ...
        *,
        retriever,
        repository: AIEvidenceRepository | None = None,
        session_repository: (
        ...
        self.repository = (
            repository or ai_evidence_repository
        )
    ...
    async def retrieve_and_persist(
        ...
        *,
        ...
        if (
            ...
            and self.session_repository.get(
                session,
                actor.tenant_id,
                session_id,
            )
        ...
        package = await self.retriever.retrieve(
            query
        )
    ...
    def to_prompt_data(
        ...
        for entry in package.entries:
            ...
            if (
                item_type
                == EvidenceType.TEXT_EXCERPT.value
            ):
