...
@router.post(
    "", response_model=MaintenanceSuccessResponse[ReliabilityProfileRead], status_code=status.HTTP_201_CREATED
)
def create_profile(
...
@router.get("", response_model=MaintenanceSuccessResponse[PageData[ReliabilityProfileRead]])
def list_profiles(
    session: SessionDep,
...
@router.get("/{identifier}", response_model=MaintenanceSuccessResponse[ReliabilityProfileRead])
def get_profile(
    identifier: int,
    session: SessionDep,
    actor: Annotated[ActorContext, Depends(require_viewer)],
...
@router.put("/{identifier}", response_model=MaintenanceSuccessResponse[ReliabilityProfileRead])
def update_profile(
    identifier: int,
    ...
    item = reliability_service.update_profile(session, actor, identifier, payload)
    return success_response(
...
@router.patch("/{identifier}/active", response_model=MaintenanceSuccessResponse[ReliabilityProfileRead])
def set_profile_active(
    ...
    item = reliability_service.set_active(session, actor, identifier, payload.is_active)
    return success_response(
        ReliabilityProfileRead.model_validate(
            item
        ),
