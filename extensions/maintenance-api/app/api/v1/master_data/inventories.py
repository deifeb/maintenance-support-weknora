...
def list_inventories(
    ...
    return success_response(
        inventory_service.list(session, **params, filters=filters), "Query completed"
    )
...
def get_inventory(identifier: int, session: SessionDep):
    return success_response(
        WarehouseInventoryRead.model_validate(inventory_service.get(session, identifier))
    )
...
def update_inventory(identifier: int, payload: WarehouseInventoryUpdate, session: SessionDep):
    return success_response(
        WarehouseInventoryRead.model_validate(
            inventory_service.update_inventory(session, identifier, payload)
        ),
        "Inventory updated",
    )
...
def adjust_inventory(identifier: int, payload: InventoryAdjustment, session: SessionDep):
    return success_response(
        WarehouseInventoryRead.model_validate(
            inventory_service.adjust(session, identifier, payload)
        ),
        "Inventory adjusted",
    )
