...
def list_warehouses(session: SessionDep, params: Annotated[dict, Depends(list_params)]):
    return success_response(warehouse_service.list(session, **params), "Query completed")
...
def delete_warehouse(identifier: int, session: SessionDep):
    warehouse_service.delete(session, identifier)
