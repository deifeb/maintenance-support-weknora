...
@router.post("", response_model=SuccessResponse[EquipmentModelRead], status_code=status.HTTP_201_CREATED)
...
def get_equipment(identifier: int, session: SessionDep):
    return success_response(EquipmentModelRead.model_validate(equipment_service.get(session, identifier)))
...
def set_equipment_active(identifier: int, payload: ActivePatch, session: SessionDep):
    ...
    return success_response(EquipmentModelRead.model_validate(item), "Equipment model status updated")
...
def delete_equipment(identifier: int, session: SessionDep):
    ...
    return success_response(DeleteResult(deleted=True, resource="equipment_model", identifier=identifier))
